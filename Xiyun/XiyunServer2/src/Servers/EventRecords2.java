...
public class EventRecords2 {
    private HashMap<String, HashMap<String, Event2>> hashMap;
    protected EventRecords2(){
        hashMap = new HashMap<>();
    ...
    protected synchronized String addEvent(String eid, String et, int capacity){
        ...
        if(!hashMap.get(et).containsKey(eid)){//if the event exists, update and return false
            hashMap.get(et).put(eid, new Event2(capacity, eid, et, null));
            return "200";
    ...
    protected synchronized String addClient(String eventId, String type){
        ...
        HashMap<String, Event2> tempHash = hashMap.get(type);
        Event2 event = tempHash.get(eventId);
        if (event != null) {
            if (event.increment()) {
    ...
    protected synchronized String listAvailability(String et){
        String s = "";
        for(Event2 ev : hashMap.get(et).values()){
